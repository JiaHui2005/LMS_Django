...
def user_list(request):
    users = User.objects.all().order_by('role', 'username').filter(is_superuser=False)
...
def user_edit(request, user_id):
    ...
    if user.id == request.user.id:
        messages.error(
            request,
            "Bạn không thể sửa tài khoản của chính mình."
        )
        return redirect('user_list')
...
def user_delete(request, user_id):
    ...
    return render(request, 'manager/user_confirm_delete.html', context)
